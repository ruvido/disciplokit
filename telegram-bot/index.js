...
class DisciploBot {
    ...
    setupHandlers() {
        ...
        this.bot.on('chat_member', async (ctx) => {
            ...
            if (oldMember.status !== 'member' && newMember.status === 'member' && inviteLink) {
            ...
            } else if (oldMember.status !== 'member' && newMember.status === 'member' && !inviteLink) {
                // NEW CASE: User joined without invite link (direct signup flow)
                console.log(`🆕 New user ${newMember.user.id} joined without invite link - generating signup token`);

                const groupName = ctx.chat.title || "Disciplo Group";
                const token = await this.config.generateSignupToken(newMember.user, groupName);

                if (token) {
                    // Get frontend URL from environment (fallback to localhost for dev)
                    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
                    const signupLink = `${frontendUrl}/signup?type=direct&token=${token}`;

                    try {
                        // Send private message with signup link
                        await this.bot.telegram.sendMessage(
                            newMember.user.id,
                            `🎉 Benvenuto in ${groupName}!\n\n` +
                            `Per completare la registrazione sulla piattaforma, clicca il link qui sotto:\n\n` +
                            `${signupLink}\n\n` +
                            `Il link è valido per 24 ore. Una volta registrato potrai accedere a tutte le funzionalità!`
                        );
                        console.log(`✅ Signup link sent to user ${newMember.user.id}`);
                    } catch (privateError) {
                        console.log(`⚠️ Could not send private message to user ${newMember.user.id}:`, privateError.message);
                        console.log('💡 User needs to start the bot first to receive private messages');
                    }
                } else {
                    console.error(`❌ Failed to generate signup token for user ${newMember.user.id}`);
                }
            } else {
                ...
                console.log(`   - Reason: Not a relevant member status change`);
            }
        });
